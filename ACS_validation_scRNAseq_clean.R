...
progressor_table <- read.csv('2023_supplementary_table2_T cell receptor repertoires associated with control and disease progression following Mycobacterium tuberculosis infection.csv',check.names = F)
...
CD4_donor_cell_count <- CD4_donor_cell_count %>% dplyr::group_by(donor_ID, days,visit, days_TB,Group) %>% dplyr::mutate(total = sum(n))
...
# Th17
CD4_progressor_table$binary <- (CD4_progressor_table$TBET <= 5) & (CD4_progressor_table$RORC > 5) 
...
ggplot(CD4_donor_cell_count,aes(x=Group, y=percentage,fill = Group))  +
  ...
  theme(text = element_text(size = 14),axis.text = element_text(size = 16),plot.title = element_text(size = 16, face = "bold")) + RotatedAxis() +
  ylab('activated CD4 %') +
  ggtitle('TBET-RORC+CD4') +
  geom_jitter(size = 1,aes(fill = Group)) +
  geom_text(aes(x = 1.5,y = 35, label = paste('p =',sprintf(wilcoxon_test_result$p.value, fmt = '%#.2f'))),size = 3) +
  scale_fill_manual(values = cols_group) + 
  theme_bw()
dev.print(pdf, paste('TB_2023_ACS_progressor_Th17_nonMAIT_CD4_alldays.pdf',sep = ''),width = 4, height = 4)
